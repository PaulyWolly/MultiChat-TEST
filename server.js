/*
  SERVER.JS
  Version: 23.0.0d
  AppName: MultiChat -TEST1-
  Updated: 05/18/2025 @12:00PM
...
if (OPENAI_API_KEY.length < 40) {
    console.error('Invalid OPENAI_API_KEY length. Key appears to be too short.');
    process.exit(1);
}
...
async function handleGPT4oMiniResponse(response, res, message, startTime) {
    ...
    try {
        ...
        if (!response || typeof response[Symbol.asyncIterator] !== 'function') {
            throw new Error('Invalid response object: not an async iterator');
        }
        ...
        if (currentParagraph.trim() && currentParagraph.trim() !== lastSentContent) {
            if (!res.writableEnded) {
                try {
                    ...
                    console.error('Error writing final content:', writeError);
                    throw writeError;
                }
            }
        }
        ...
        if (!res.writableEnded) {
            ...
            try {
                ...
                console.error('Error sending completion signal:', writeError);
                throw writeError;
            }
        }
    } catch (error) {
